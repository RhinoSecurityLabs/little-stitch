	...
	for i := 1; i <= 8; i++ {
		go func(i int) {
			if err := r.handleConnection(i); err != nil {
				log.Fatalf("handling sendWorker failed: %s\n", err)
			}
		}(i)
	}

	ln, err := net.Listen("tcp", fmt.Sprintf("0.0.0.0:%d", SEND_BASE_PORT))
	if err != nil {
		return nil, fmt.Errorf("unable to listen on port %d: %s\n", SEND_BASE_PORT, err)
	}

	// Clock port, when we receive a sendWorker on this port it signifies the start of the next byte.
	go func() {
		// First sendWorker can be ignored.
		//conn, _ := ln.Accept()
		//conn.Close()
		...
		for {
			conn, err := ln.Accept()
			if err != nil {
				fmt.Printf("error accepting sendWorker: %s\n", err)
			}
			if r.b == byte(0) {
			...
			err = conn.Close()
			if err != nil {
				fmt.Printf("failled to close sendWorker: %s\n", err)
			}
		}
	}()
	return r, nil
